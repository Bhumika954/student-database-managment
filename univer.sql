...
SELECT 
    s.st_id,
    s.name,
    COUNT(a.atten) AS total_classes,
    SUM(CASE WHEN a.atten = 'present' THEN 1 ELSE 0 END) AS present_count,
    (SUM(CASE WHEN a.atten = 'present' THEN 1 ELSE 0 END) / COUNT(a.atten)) * 100 AS attendance_percentage
FROM 
    at_student s
JOIN 
    attn a ON s.st_id = a.st_id
WHERE 
    a.at_date != '0000-00-00' -- Ignoring invalid dates
GROUP BY 
    s.st_id, s.name;
